<!-- 
 * File: searchUser.php SWE381 - Project
 * EDIT DATE: 5/16/2024 
 * AUTHORS: 
...
if (isset($_POST['input']) && isset($_SESSION['username'])) {
    $currentUser = $_SESSION['username'];
    $input = $_POST['input'];
    $type = $_POST['scope'];

    if ($type == 'questions') {
        $sql = "SELECT * FROM question WHERE title LIKE '%{$input}%' AND user_name = '$currentUser'";
        ...
        ";
            ...
            echo "<h1 class='text-lg font-bold'>No Questions Found</h1>";
        }
    } else {
        $sql = "SELECT * FROM answers WHERE answerText LIKE '%{$input}%' AND user_name = '$currentUser'";
        $result = $conn->query($sql);
        if (mysqli_num_rows($result) > 0) {
            while ($row = $result->fetch_assoc()) {
        ...
        <div class='absolute bottom-2 right-2 flex items-center space-x-2'>
            ...
            echo "<h1 class='text-lg font-bold'>No Answers Found</h1>";
        }
    }
}
?>
